...
if (!empty($_POST)) {
  ...
  if ($_POST['action'] == 'infoProducto') {
    ...
    $query = mysqli_query($conexion, "SELECT codproducto, nombre_producto, precio, precio_iva, stock FROM producto WHERE codproducto = $producto_id");
    ...
    while ($data = mysqli_fetch_assoc($query_detalle_temp)) {
      ...
      $total = round($total + $precioTotal, 2);
        ...
        $detalleTabla .='<tr>
            ...
            <td>
                <a href="#" class="btn btn-primary" onclick="event.preventDefault(); del_product_detalle('.$data['correlativo'].');"><i class="fas fa-trash-alt"></i> Eliminar</a>
            </td>
        </tr>';
    }
    $impuesto = round($sub_total / $igv, 2);
...
if ($_POST['action'] == 'searchForDetalle') {
  ...
  if (empty($_POST['user'])){
    ...
    if ($result > 0) {
    if ($result_igv > 0) {
      $info_iva = mysqli_fetch_assoc($query_igv);
      $iva = $info_igv['igv'];
    }

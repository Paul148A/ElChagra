...
if (!empty($_POST)) {
    $alert = "";
    if  (empty($_POST['nombre']) || empty($_POST['telefono']) ||  empty($_POST['celular']) || empty($_POST['direccion'])) {
        ...
        if ($result > 0) {
            ...
            $query_insert = mysqli_query($conexion, "INSERT INTO cliente(dni,nombre,telefono,celular,direccion, usuario_id) values ('$dni', '$nombre', '$telefono', '$celular', '$direccion', '$usuario_id')");
            if ($query_insert) {
                $alert = '<div class="alert alert-primary" role="alert">
                                    Cliente Registrado
                                </div>';
